...
// Escaleras y serpientes del tablero
const ESCALERAS = {
...
// ═════════════════════════════════════════════════════════
//  PANTALLA INICIO — partículas + config
// ═════════════════════════════════════════════════════════
...
function mostrarAnimacionSerpiente(jugadorNombre, desde, hasta) {
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      const jugador = overlay.querySelector(".jugador-resbalando");
      if (jugador) jugador.classList.add("cayendo");
    }, 500);
...
// ═════════════════════════════════════════════════════════
//  ANIMACIÓN DE MOVIMIENTO MÁS LENTA Y VISIBLE
// ═════════════════════════════════════════════════════════
...
async function animarFichaMovimiento(jugador, desde, hasta, colorIdx, color) {
  ...
  flotante.style.cssText = `
    ...
    box-shadow: 0 0 12px ${colorFicha}, 0 3px 8px rgba(0,0,0,0.6);
    pointer-events: none;
    z-index: 100;
    transition: none;
  ...
  for (let i = 1; i < ruta.length; i++) {
    ...
    const tableroOffsetX = tableroRect.left - wrapRect.left;
    const tableroOffsetY = tableroRect.top - wrapRect.top;

    flotante.style.transition = `left ${stepMs}ms cubic-bezier(0.4, 0, 0.2, 1), top ${stepMs}ms cubic-bezier(0.4, 0, 0.2, 1)`;
    flotante.style.left = (pos.x + tableroOffsetX) + "px";
    flotante.style.top = (pos.y + tableroOffsetY) + "px";

    const casEl = $("cas-" + ruta[i]);
    if (casEl) {
      casEl.classList.add("casilla-paso");
      setTimeout(() => casEl.classList.remove("casilla-paso"), stepMs * 1.5);
    }

    await sleep(stepMs);
  }

  flotante.style.transition = "transform 0.2s ease";
...
async function animarEspecial(casilla, tipo, color) {
  const el = $("cas-" + casilla);
  if (!el) return;

  if (tipo === "ESCALERA") {
    el.classList.add("casilla-escalera-anim");
    await sleep(800);
    el.classList.remove("casilla-escalera-anim");
  } else if (tipo === "SERPIENTE") {
    el.classList.add("casilla-serpiente-anim");
    await sleep(800);
    el.classList.remove("casilla-serpiente-anim");
  }
}
...
async function animarMovimientoJugadores(data) {
  ...
  for (const { jugador, desde, hasta, colorIdx } of movimientos) {
    ...
    // Usar el evento del servidor si corresponde a este jugador
    let especial = null;
    let casillaTrigger = desde;
    let destinoEspecial = hasta;

    if (movimiento && movimiento.jugador === jugador.nombre) {
      if (movimiento.evento === "escalera") {
        especial = "ESCALERA";
        casillaTrigger = movimiento.desde;
        destinoEspecial = movimiento.hasta;
      } else if (movimiento.evento === "serpiente") {
        especial = "SERPIENTE";
        casillaTrigger = movimiento.desde;
        destinoEspecial = movimiento.hasta;
      }
    } else {
      // Fallback: buscar en mapas locales
      if (ESCALERAS[desde] === hasta) {
        especial = "ESCALERA";
        destinoEspecial = hasta;
      } else if (SERPIENTES[desde] === hasta) {
        especial = "SERPIENTE";
        destinoEspecial = hasta;
      }
    }
    ...
    if (especial) {
      ...
      // Mostrar animación épica según el tipo
      if (especial === "ESCALERA") {
        await mostrarAnimacionEscalera(jugador.nombre, casillaTrigger, destinoEspecial);
      } else {
        await mostrarAnimacionSerpiente(jugador.nombre, casillaTrigger, destinoEspecial);
      }
      
      await animarEspecial(casillaTrigger, especial, color);
      await sleep(500);
...
// ── Ciclo bots más lento ────────────────────────────────────────────
async function procesarTurnoAutomatico() {
  while (estado?.esBot && !estado?.ganador && !estado?.esperandoRespuesta) {
    if (procesandoTurno) return;
    procesandoTurno = true;
...
async function responder() {
  ...
  try {
    ...
    mostrarFeedback(ok ? "✅" : "❌",
      ok ? "¡Correcto! Ganas un turno extra." : `Incorrecto. Era: "${data.respuestaCorrecta}"`,
    ...
    if (data.ganador) { setTimeout(() => mostrarGanador(data), 1800); return; }
...
function actualizarEstado(data) {
  ...
  renderRanking();
  actualizarHdr();
  if (data.esperandoRespuesta && data.reto) mostrarReto(data.reto, data.turnoActual);
}
...
// ═════════════════════════════════════════════════════════
//  BANNER DE TURNO CON COLOR DEL JUGADOR
// ═════════════════════════════════════════════════════════

let bannerTimer = null;

function mostrarBannerTurno(data, callback) {
  if (!data || data.ganador || data.esperandoRespuesta) { callback?.(); return; }
...
function turnoBannerOcultar() {
  turnoBanner.classList.add("hidden");
  turnoInner?.classList.remove("saliendo");
}
...
async function actualizarHistorial() {
  try {
    ...
    list.innerHTML = "";
    data.historial?.slice(0, 10).forEach((e, i) => {
...
function mostrarReto(reto, jugador) {
